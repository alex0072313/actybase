    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-body panel-form">
                    <form action="{{ route('_user_update', ['user' => $user->id]) }}" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered" data-parsley-validate="true">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <select name="role" class="default-select2 form-control">
                                    @foreach(\Spatie\Permission\Models\Role::all() as $role)
                                        @continue($role->name == 'megaroot')
                                        <option {{ $user->hasRole($role->name) ? 'selected' : '' }} value="{{ $role->name }}">{{ config('role.names.'.$role->name.'.dolg') }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            <div class="clearfix">
                                ...
                                <a href="{{ route('_user_destroy', $user->id) }}" data-click="swal-warning" data-title="Подтвердите действие" data-text="Удалить пользователя {{ $user->name }}?" data-classbtn="danger" data-actionbtn="Удалить" data-type="error" class="btn btn-sm btn-danger float-right">Удалить</a>
                            </div>
                        </div>
        ...
        <div class="col-lg-6">
            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-body panel-form">
                    <form action="{{ route('_user_update', ['user' => $user->id]) }}" method="post" enctype="multipart/form-data" class="form-horizontal form-bordered">
                        @csrf
